...
def process_documents(ignored_files=[]):
    ...
    # chunk_size and chunk_overlap were defined earlier.
    ...
    for doc in documents:
        ...
        for split in splits:
            texts.append(Document(page_content=split, metadata={"source": doc.metadata["source"]}))
...
def main():
    ...
    start_time = time.time()
    if does_vectorstore_exist(PERSIST_DIRECTORY, os.path.basename(EMBEDDINGS_MODEL_DIR)):
        ...
        texts = process_documents(ignored_files=existing_sources)
